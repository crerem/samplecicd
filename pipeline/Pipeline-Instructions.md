...
aws ssm put-parameter \
    --name "/service/sam-pipeline-start-cicd/github/repo" \
    --description "Github Repository name for Cloudformation Stack sam-pipeline-start-cicd-pipeline" \
    --type "String" \
    --value "GITHUB_REPO_NAME"

aws ssm put-parameter \
    --name "/service/sam-pipeline-start-cicd/github/token" \
    --description "Github Token for Cloudformation Stack sam-pipeline-start-cicd-pipeline" \
    --type "String" \
    --value "TOKEN"

aws ssm put-parameter \
    --name "/service/sam-pipeline-start-cicd/github/user" \
    --description "Github Username for Cloudformation Stack sam-pipeline-start-cicd-pipeline" \
    --type "String" \
    --value "GITHUB_USER"
```

**NOTE:** Keep in mind that these Parameters will only be available within the same region you're deploying this Pipeline stack. Also, if these values ever change you will need to [update these parameters](https://docs.aws.amazon.com/cli/latest/reference/ssm/put-parameter.html) as well as update the "sam-pipeline-start-cicd-pipeline" Cloudformation stack.
...
aws cloudformation create-stack \
    --stack-name sam-pipeline-start-cicd-pipeline \
